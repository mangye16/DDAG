...
from mutual_att_model_T import embed_net
from utils import *
...
parser.add_argument('--model_path', default='save_model_sgd/', type=str, help='model save path')
...
parser.add_argument('--lcpa', action='store_true', help='either add local part attention')
...
if dataset == 'sysu':
    ...
    data_path = '../Datasets/re-id/RegDB/'
    n_class = 206
    test_mode = [2, 1]
...
net = embed_net(args.low_dim, n_class, drop = args.drop, part = args.part, arch=args.arch, cpa  =args.lcpa)
net.to(device)    
...
for trial in range(10):
    ...
    # pool5 feature
    distmat_pool = np.matmul(query_feat_att, np.transpose(gall_feat_att))
